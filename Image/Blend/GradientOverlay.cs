...
public class GradientOverlayEffect : BaseBlendEffect
{
    ...
    protected override void OnPropertyChanged(DependencyPropertyChangedEventArgs e)
    {
        ...
        if (e.Property == MaximumProperty || e.Property == MinimumProperty || e.Property == ValueProperty)
            SetCurrentValue(ProgressProperty, new Range<double>(Minimum, Maximum).ToRange(0, 1, Value));
    }
    ...
    private static void OnGradientChanged(DependencyObject sender, DependencyPropertyChangedEventArgs e) => sender.As<GradientOverlayEffect>().OnGradientChanged(new((Gradient)e.OldValue, (Gradient)e.NewValue));
    ...
    private static void OnTypeChanged(DependencyObject sender, DependencyPropertyChangedEventArgs e) => sender.As<GradientOverlayEffect>().TypeIndex = (int)(GradientType)e.NewValue;
    ...
    */
    ...
    public GradientOverlayEffect() : base()
    {
        ...
        var colors = XWriteableBitmap.New(Gradient.Steps.Count, 1);
        ...
        var offsets = XWriteableBitmap.New(Gradient.Steps.Count, 1);
    ...
    public static Matrix<Color> RenderAngle(Gradient input, MSize<int> size, double angle)
    {
        int cx = size.Width / 2, cy = size.Height / 2;

        GradientStep a = null, b = null;

        var result = new Color[size.Height, size.Width];
        for (var y = 0; y < size.Height; y++)
        {
            for (var x = 0; x < size.Width; x++)
            {
                ...
                var t = Math.Atan2(y - cy, x - cx) + new Numeral.Angle(angle).Convert(AngleType.Radian);
                t += Math.PI;
                if (t > r)
                    t -= r;
                ...
                var q = Color.Add(Color.Multiply(XColor.Convert(a.Color), Convert.ToSingle(p)), Color.Multiply(XColor.Convert(b.Color), Convert.ToSingle(1 - p)));
                result[y, x] = Color.FromArgb(255, q.R, q.G, q.B);
            }
        }
        return new Matrix<Color>(result.As());
    }

    /// <remarks>To do: Observe angle.</remarks>
    public static Matrix<Color> RenderCircle(Gradient input, MSize<int> size, double angle)
    {
        ...
        for (int y = 0; y < size.Height; y++)
        {
            for (int x = 0; x < size.Width; x++)
            {
                ...
                var s = System.Convert.ToByte(FLerp(nd, 255, 0));
                var t = Convert.ToDouble(s) / 255.0;

                GetColors(input, t, ref aStep, ref bStep);
                if (aStep is null || bStep is null)
                    continue;

                var p = (t - aStep.Offset) / (bStep.Offset - aStep.Offset);

                var ca = XColor.Convert(aStep.Color);
                var cb = XColor.Convert(bStep.Color);
                cb = Color.FromArgb(Convert.ToByte(p * 255d), cb.R, cb.G, cb.B);

                var va = new ByteVector4(ca.R, ca.G, ca.B, ca.A);
                var vb = new ByteVector4(cb.R, cb.G, cb.B, cb.A);
                var vc = va.Blend(vb);

                result[y, x] = Color.FromArgb(vc.A, vc.R, vc.G, vc.B);
            }
        }
        return new Matrix<Color>(result.As());
    }
    ...
    /// <remarks>To do: Observe angle.</remarks>
    public static Matrix<Color> RenderDiamond(Gradient input, MSize<int> size, double angle)
    {
        int cx = size.Width / 2, cy = size.Height / 2;
        ...
        double P()
        {
            ...
            var t = Convert.ToDouble(System.Convert.ToByte(FLerp(dn, 255, 0))) / 255.0;
            GetColors(input, t, ref aStep, ref bStep);
            if (aStep is null || bStep is null)
                return 0;
    ...
    /// <remarks>To do: Observe angle.</remarks>
    public static Matrix<Color> RenderLinear(Gradient input, MSize<int> size, double angle)
    {
        GradientStep aStep = null, bStep = null;
        ...
        for (int y = 0; y < size.Height; y++)
        {
            for (int x = 0; x < size.Width; x++)
            {
                var t = Convert.ToDouble(x) / Convert.ToDouble(size.Width);
                ...
                var ca = XColor.Convert(aStep.Color);
                var cb = XColor.Convert(bStep.Color);
                cb = Color.FromArgb(Convert.ToByte(Math.Clamp(p * 255d, 0, 255)), cb.R, cb.G, cb.B);

                var va = new ByteVector4(ca.R, ca.G, ca.B, ca.A);
                var vb = new ByteVector4(cb.R, cb.G, cb.B, cb.A);
                var vc = va.Blend(vb);

                var color = Color.FromArgb(vc.A, vc.R, vc.G, vc.B);
                result[y, x] = color;
            }
        }
        return new Matrix<Color>(result.As());
    ...
    private static float FLerp(float norm, float min, float max)
        => (max - min) * norm + min;

    private static void GetColors(Gradient input, double t, ref GradientStep a, ref GradientStep b)
    {
        var count = input.Steps.Count;
        for (var i = count - 1; i >= 0; i--)
        {
    ...
    public static GradientStep[] GetSteps(Gradient gradient, bool reflect, bool reverse)
    {
        ...
        if (reflect)
        {
            var oldRange = new Range<double>(0, 1);
            foreach (var i in result)
                i.Offset = oldRange.ToRange(0, 0.5, i.Offset);
    ...
    private void OnGradientPropertySet(IPropertySet sender, PropertySetEventArgs e) => Update(Gradient);

    private void Register(Gradient gradient, int i)
    {
    ...
    private void Update(Gradient gradient)
    {
    ...
    protected virtual void OnGradientChanged(ValueChange<Gradient> input)
    {
        input.OldValue.IfNotNull(i =>
        {
            i.PropertySet -= OnGradientPropertySet;
        });
        input.NewValue.IfNotNull(i =>
        {
            i.PropertySet += OnGradientPropertySet;
    ...
    {
        if (Gradient is null || Gradient.Steps.Count == 0)
            return default;
        ...
        Try.Do(() =>
        {
            ...
            var size = new MSize<int>(Convert.ToInt32(input.PixelHeight * Scale), Convert.ToInt32(input.PixelWidth * Scale));
            switch (Type)
            {

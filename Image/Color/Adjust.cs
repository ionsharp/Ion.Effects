...
public class AdjustEffect : BaseBlendEffect
{
    ...
    public override Color Apply(Color color, double amount = 1)
    {
        ...
        var nr = Convert.ToByte(Math.Clamp((r * 0.393) + (g * 0.769) + (b * 0.189), 0, 255));
        var ng = Convert.ToByte(Math.Clamp((r * 0.349) + (g * 0.686) + (b * 0.168), 0, 255));
        var nb = Convert.ToByte(Math.Clamp((r * 0.272) + (g * 0.534) + (b * 0.131), 0, 255));
